...
def print_help():
	print '  Add_cdo_info_to_wrfout.d01.py: Add necessary information to the WRF output, so that the coordinate system (LCC) can be recognized by CDO `cdo sinfo`. Only for d01 domain.'
	print '  Ver:     1.0.0'
	print '  Author:  Xiaodong Chen <xiaodc.work@gmail.com>\n'
	print '  Use: Add_cdo_info_to_wrfout.d01.py <geoinfo_file> <wrffile> <outfile> <start_date> <start_hour> <frames>\n'
	print '         <geoinfo_file>    geogrid.exe output containing the half-resolution grids information'
	print '                           e.g. For 15km grids WRF output, this file should contain the 7.5km grids info of the same domain'
	...
	print '         <wrffile>         WRF variable data after priliminary post-processing, or WRF output (i.e. wrfout_d01* file)'
	...
	print '         <start_hour>      The first hour in the <wrffile>.\n'
        print '         <frames>          Total time steps in the WRF output. Get the info from "ncdump -h"\n'
        print '  Note: For more detailed instructions and example, plesea check the README.md file in this folder\n'
...
for i in np.arange(nx):
	for j in np.arange(ny):
		...
		lat_ll[i,j] = lat_matrix_half[i*2,j*2]
		lat_lr[i,j] = lat_matrix_half[i*2,j*2+2]
		lat_ur[i,j] = lat_matrix_half[i*2+2,j*2+2]
		lat_ul[i,j] = lat_matrix_half[i*2+2,j*2]
        
		lon_center[i,j] = lon_matrix[i,j]
		lon_ll[i,j] = lon_matrix_half[i*2,j*2]
		lon_lr[i,j] = lon_matrix_half[i*2,j*2+2]
		lon_ur[i,j] = lon_matrix_half[i*2+2,j*2+2]
		lon_ul[i,j] = lon_matrix_half[i*2+2,j*2]
